...
[[ ! -f /etc/rspamd/override.d/worker-controller-password.inc ]] && echo '# Autogenerated by maimail' > /etc/rspamd/override.d/worker-controller-password.inc

echo ${IPV4_NETWORK}.0/24 > /etc/rspamd/custom/maimail_networks.map
echo ${IPV6_NETWORK} >> /etc/rspamd/custom/maimail_networks.map
...
timeout = 10;
EOF
  until [[ $(redis-cli -h redis-maimail -a ${REDISPASS} --no-auth-warning PING) == "PONG" ]]; do
    echo "Waiting for Redis @redis-maimail..."
    sleep 2
  done
  ...
  redis-cli -h redis-maimail -a ${REDISPASS} --no-auth-warning SLAVEOF ${REDIS_SLAVEOF_IP} ${REDIS_SLAVEOF_PORT}
else
  cat <<EOF > /etc/rspamd/local.d/redis.conf
servers = "redis:6379";
password = "${REDISPASS}";
...
EOF
  ...
  redis-cli -h redis-maimail -a ${REDISPASS} --no-auth-warning SLAVEOF NO ONE
fi
...
# These exists in maimail UI and should not be removed
touch /etc/rspamd/custom/global_mime_from_blacklist.map \
  ...
  /etc/rspamd/custom/maimail_networks.map \
...
# If DQS KEY is set in maimail.conf add Spamhaus DQS RBLs
if [[ ! -z ${SPAMHAUS_DQS_KEY} ]]; then
    cat <<EOF > /etc/rspamd/custom/dqs-rbl.conf
  # Autogenerated by maimail. DO NOT TOUCH!
    spamhaus {
