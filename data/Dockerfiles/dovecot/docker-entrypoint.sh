...
echo ${RAND_USER}@maimail.local:{SHA1}$(echo -n ${RAND_PASS} | sha1sum | awk '{print $1}'):::::: > /etc/dovecot/dovecot-master.passwd
echo ${RAND_USER}@maimail.local::5000:5000:::: > /etc/dovecot/dovecot-master.userdb
echo ${RAND_USER}@maimail.local:${RAND_PASS} > /etc/sogo/sieve.creds
...
echo -n ${RAND_USER}@maimail.local:${RAND_PASS} > /etc/sogo/cron.creds
...
# Setting variables for indexer-worker inside fts.conf automatically according to maimail.conf settings
