...
if (isset($_POST["verify_tfa_login"])) {
  if (verify_tfa_login($_SESSION['pending_maimail_cc_username'], $_POST)) {
    if ($_SESSION['pending_maimail_cc_role'] == "admin") {
      $_SESSION['maimail_cc_username'] = $_SESSION['pending_maimail_cc_username'];
...
if (isset($_POST["verify_fido2_login"])) {
  ...
  if (is_array($res) && $res['role'] == "admin" && !empty($res['username'])){
    $_SESSION["maimail_cc_username"] = $res['username'];
    $_SESSION["maimail_cc_role"] = $res['role'];
...
if (isset($_POST["login_user"]) && isset($_POST["pass_user"])) {
  ...
  if ($as == "admin") {
    session_regenerate_id(true);
		$_SESSION['maimail_cc_username'] = $login_user;
		$_SESSION['maimail_cc_role'] = "admin";
		header("Location: /admin/dashboard");
    die();
	}
...
if (isset($_SESSION['maimail_cc_role']) && $_SESSION['maimail_cc_role'] == "admin" && !isset($_SESSION['maimail_cc_api'])) {
