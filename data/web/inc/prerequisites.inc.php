...
// OAuth2
class maimailPdo extends OAuth2\Storage\Pdo {
...
$oauth2_storage = new maimailPdo(array('dsn' => $dsn, 'username' => $database_user, 'password' => $database_pass));
...
// Set language
if (!isset($_SESSION['maimail_locale']) && !isset($_COOKIE['maimail_locale'])) {
  if ($DETECT_LANGUAGE && isset($_SERVER['HTTP_ACCEPT_LANGUAGE'])) {
    ...
    foreach ($lang2pref as $lang => $q) {
      if (array_key_exists($lang, $AVAILABLE_LANGUAGES)) {
        $_SESSION['maimail_locale'] = $lang;
        break;
      } elseif (array_key_exists($lang, $AVAILABLE_BASE_LANGUAGES)) {
        $_SESSION['maimail_locale'] = $AVAILABLE_BASE_LANGUAGES[$lang];
        break;
      }
    }
    ...
    if (!isset($_SESSION['maimail_locale'])) {
      foreach ($lang2pref as $lang => $q) {
        if (array_key_exists(substr($lang, 0, 2), $AVAILABLE_BASE_LANGUAGES)) {
          $_SESSION['maimail_locale'] = $AVAILABLE_BASE_LANGUAGES[substr($lang, 0, 2)];
          break;
        }
      }
    }
    ...
    $_SESSION['maimail_locale'] = strtolower(trim($DEFAULT_LANG));
  }
}
if (isset($_COOKIE['maimail_locale'])) {
  (preg_match('/^[a-z]{2}$/', $_COOKIE['maimail_locale'])) ? $_SESSION['maimail_locale'] = $_COOKIE['maimail_locale'] : setcookie("maimail_locale", "", time() - 300);
}
if (isset($_GET['lang']) && array_key_exists($_GET['lang'], $AVAILABLE_LANGUAGES)) {
  $_SESSION['maimail_locale'] = $_GET['lang'];
  setcookie("maimail_locale", $_GET['lang'], time()+30758400); // one year
}
...
$langFile = $_SERVER['DOCUMENT_ROOT'] . '/lang/lang.'.$_SESSION['maimail_locale'].'.json';
...
require_once $_SERVER['DOCUMENT_ROOT'] . '/inc/functions.llm.inc.php';
...
if (isset($_SESSION['maimail_cc_role'])) {
  // if ($_SESSION['maimail_cc_role'] == 'user') {
    ...
    // $imap_connection = $imap_server->authenticate($_SESSION['maimail_cc_username'] . '*' . trim($master_user), trim($master_passwd));
