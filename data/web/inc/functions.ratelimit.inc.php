...
function ratelimit($_action, $_scope, $_data = null, $_extra = null) {
  ...
  switch ($_action) {
      ...
      switch ($_scope) {
        case 'domain':
          ...
          foreach ($objects as $object) {
            ...
            if (!hasDomainAccess($_SESSION['maimail_cc_username'], $_SESSION['maimail_cc_role'], $object)) {
              $_SESSION['return'][] = array(
                'type' => 'danger',
                'log' => array(__FUNCTION__, $_action, $_scope, $_data_log),
                'msg' => 'access_denied'
        ...
        case 'mailbox':
          ...
          foreach ($objects as $object) {
            ...
            if (!hasMailboxObjectAccess($_SESSION['maimail_cc_username'], $_SESSION['maimail_cc_role'], $object)
                || ($_SESSION['maimail_cc_role'] != 'admin' && $_SESSION['maimail_cc_role'] != 'domainadmin' && $_SESSION['access_all_exception'] != '1')) {
              $_SESSION['return'][] = array(
                'type' => 'danger',
                'log' => array(__FUNCTION__, $_action, $_scope, $_data_log),
                'msg' => 'access_denied'
      ...
      switch ($_scope) {
        ...
        case 'mailbox':
          if (!hasMailboxObjectAccess($_SESSION['maimail_cc_username'], $_SESSION['maimail_cc_role'], $_data)
            || ($_SESSION['maimail_cc_role'] != 'admin' && $_SESSION['maimail_cc_role'] != 'domainadmin')) {
            return false;
          }
          try {
            if ($rl_value = $redis->hGet('RL_VALUE', $_data)) {
    ...
    case 'delete':
      $data['hash'] = $_data;
      if ($_SESSION['maimail_cc_role'] != 'admin' || !preg_match('/^RL[0-9A-Za-z=]+$/i', trim($data['hash']))) {
        $_SESSION['return'][] = array(
          'type' => 'danger',
          'log' => array(__FUNCTION__, $_action, $_type, $_data_log, $_attr),
          'msg' => 'access_denied'
