  ...
  switch ($_action) {
    case 'add':
      ...
      if (is_valid_domain_name($local_dest)) {
        if (!hasDomainAccess($_SESSION['maimail_cc_username'], $_SESSION['maimail_cc_role'], $local_dest)) {
          $_SESSION['return'][] = array(
            'type' => 'danger',
            'log' => array(__FUNCTION__, $_action, $_data, $_attr),
            'msg' => 'access_denied'
      ...
      elseif (filter_var($local_dest, FILTER_VALIDATE_EMAIL)) {
        ...
        if (!hasMailboxObjectAccess($_SESSION['maimail_cc_username'], $_SESSION['maimail_cc_role'], $local_dest) &&
          !hasAliasObjectAccess($_SESSION['maimail_cc_username'], $_SESSION['maimail_cc_role'], $local_dest)) {
            $_SESSION['return'][] = array(
              'type' => 'danger',
              'log' => array(__FUNCTION__, $_action, $_data, $_attr),
              'msg' => 'access_denied'
    ...
    case 'details':
      ...
      if (!hasDomainAccess($_SESSION['maimail_cc_username'], $_SESSION['maimail_cc_role'], $bccdata['domain'])) {
        $bccdata = null;
        return false;
      }
      return $bccdata;
    ...
    case 'get':
      ...
      foreach ($all_items as $i) {
        if (hasDomainAccess($_SESSION['maimail_cc_username'], $_SESSION['maimail_cc_role'], $i['domain'])) {
          $bccdata[] = $i['id'];
        }
      }
      $all_items = null;
