...
function mailq($_action, $_data = null) {
  ...
  if ($_action == 'get') {
    $mailq_lines = docker('post', 'postfix-maimail', 'exec', array('cmd' => 'mailq', 'task' => 'list'));
    $lines = 0;
  ...
  elseif ($_action == 'delete') {
    ...
    $docker_return = docker('post', 'postfix-maimail', 'exec', array('cmd' => 'mailq', 'task' => 'delete', 'items' => $qids));
  ...
  elseif ($_action == 'cat') {
    ...
    $docker_return = docker('post', 'postfix-maimail', 'exec', array('cmd' => 'mailq', 'task' => 'cat', 'items' => $qids));
  ...
  elseif ($_action == 'edit') {
    if (in_array($_data['action'], array('hold', 'unhold', 'deliver'))) {
      ...
      if (!empty($qids)) {
        $docker_return = docker('post', 'postfix-maimail', 'exec', array('cmd' => 'mailq', 'task' => $_data['action'], 'items' => $qids));
        process_mailq_output(json_decode($docker_return, true), $_action, $_data);
      }
    }
    if (in_array($_data['action'], array('flush', 'super_delete'))) {
      $docker_return = docker('post', 'postfix-maimail', 'exec', array('cmd' => 'mailq', 'task' => $_data['action']));
      process_mailq_output(json_decode($docker_return, true), $_action, $_data);
    }
  }
}
