  ...
  switch ($_action) {
    ...
    case 'quick_release':
      ...
      if (empty($row['id']) || !is_numeric($row['id'])) {
        ...
        if (!empty(gethostbynamel('postfix-maimail'))) {
          $postfix = 'postfix-maimail';
        }
    ...
    case 'delete':
      ...
      foreach ($ids as $id) {
        ...
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!hasMailboxObjectAccess($_SESSION['maimail_cc_username'], $_SESSION['maimail_cc_role'], $row['rcpt']) && $_SESSION['maimail_cc_role'] != 'admin') {
          $_SESSION['return'][] = array(
            'type' => 'danger',
            'log' => array(__FUNCTION__, $_action, $_data_log),
            'msg' => 'access_denied'
    ...
    case 'edit':
      ...
      elseif ($_data['action'] == 'release' || $_data['action'] == 'learnham') {
        ...
        foreach ($ids as $id) {
          ...
          $stmt->execute(array(':id' => $id));
          $row = $stmt->fetch(PDO::FETCH_ASSOC);
          if (!hasMailboxObjectAccess($_SESSION['maimail_cc_username'], $_SESSION['maimail_cc_role'], $row['rcpt']) && $_SESSION['maimail_cc_role'] != 'admin' || empty($row['rcpt'])) {
            $_SESSION['return'][] = array(
              'type' => 'danger',
              'log' => array(__FUNCTION__, $_action, $_data_log),
              'msg' => 'access_denied'
    ...
    case 'get':
      if ($_SESSION['maimail_cc_role'] == "user") {
        ...
        $stmt->execute(array(':mbox' => $_SESSION['maimail_cc_username']));
