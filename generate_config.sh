...
# Ensure script is executed in the maimail installation directory by checking for a .env symlink that points to maimail.conf
if [ ! -L "${PWD}/.env" ]; then
  echo -e "\e[33mPlease run this script from the maimail installation directory.\e[0m"
  echo -e "  \e[36mcd /path/to/maimail && ./generate_config.sh\e[0m"
  exit 1
fi

# Verify the .env symlink points to a maimail.conf file
env_target="$(readlink -f "${PWD}/.env" 2>/dev/null || true)"
if [ -z "$env_target" ] || [ "$(basename "$env_target")" != "maimail.conf" ]; then
  echo -e "\e[31mThe found .env symlink does not point to a maimail.conf file.\e[0m"
  echo -e "\e[33mPlease create a symbolic link .env -> maimail.conf inside the maimail directory and run this script there.\e[0m"
  echo -e "\e[33mNote: 'ln -s maimail.conf .env' will create the symlink even if maimail.conf does not yet exist.\e[0m"
  echo -e "  \e[36mcd /path/to/maimail && ln -s maimail.conf .env && ./generate_config.sh\e[0m"
  exit 1
fi

# Load maimail Generic Scripts
...
if [[ $docker_version -lt 24 ]]; then
  ...
  echo -e "\e[33mmaimail needs a newer Docker version to work properly...\e[0m"
...
if [ -z "${SKIP_CLAMD}" ]; then
  if [ "${MEM_TOTAL}" -le "2621440" ]; then
    ...
    echo "ClamAV can be re-enabled by setting SKIP_CLAMD=n in maimail.conf."
...
if [[ ${SKIP_BRANCH} != y ]]; then
  echo "Which branch of maimail do you want to use?"
  echo ""
...
cat << EOF > maimail.conf
# ------------------------------
# maimail web ui configuration
# ------------------------------
...
# see https://docs.maimail.email/models/model-passwd/
...
# For IPv6 see https://docs.maimail.email/post_installation/firststeps-ip_bindings/
HTTP_PORT=80
HTTP_BIND=
...
COMPOSE_PROJECT_NAME=maimaildockerized

# Used Docker Compose version
# Switch here between native (compose plugin) and standalone
# For more information take a look at the maimail docs regarding the configuration options.
# Normally this should be untouched but if you decided to use either of those you can switch it manually here.
# Please be aware that at least one of those variants should be installed on your machine or maimail will fail.
...
#
# You can use wildcard records to create specific names for every domain you add to maimail.
# Example: Add domains "example.com" and "example.net" to maimail, change ADDITIONAL_SAN to a value like:
...
# between services. So acme-maimail obtains for maildomains and all web-things get handled
# in the reverse proxy.
AUTODISCOVER_SAN=y

# Additional server names for maimail UI
#
# Specify alternative addresses for the maimail UI to respond to
# This is useful when you set mail.* as ADDITIONAL_SAN and want to make sure mail.maildomain.com will always point to the maimail UI.
# If the server name does not match a known site, Nginx decides by best-guess and may redirect users to the wrong web root.
# You can understand this as server_name directive in Nginx.
# Comma separated list without spaces! Example: ADDITIONAL_SERVER_NAMES=a.b.c,d.e.f
ADDITIONAL_SERVER_NAMES=

# Skip running ACME (acme-maimail, Let's Encrypt certs) - y/n
SKIP_LETS_ENCRYPT=n
...
# Skip ClamAV (clamd-maimail) anti-virus (Rspamd will auto-detect a missing ClamAV container) - y/n
SKIP_CLAMD=${SKIP_CLAMD}

# Skip Olefy (olefy-maimail) anti-virus for Office documents (Rspamd will auto-detect a missing Olefy container) - y/n
SKIP_OLEFY=n
...
# Dovecot inside maimail use Flatcurve as FTS Backend.
SKIP_FTS=n
...
# Enable watchdog (watchdog-maimail) to restart unhealthy containers
USE_WATCHDOG=y
...
#WATCHDOG_NOTIFY_WEBHOOK_BODY='{"username": "maimail Watchdog", "content": "**${SUBJECT}**\n${BODY}"}'
...
# Checks if maimail is an open relay. Requires a SAL. More checks will follow.
# https://www.servercow.de/maimail?lang=en
# https://www.servercow.de/maimail?lang=de
# No data is collected. Opt-in and anonymous.
# Will only work with unmodified maimail setups.
...
# User expands to DOVECOT_MASTER_USER@maimail.local
...
# root certificates can be placed for validation under maimail-dockerized/data/web/inc/lib/WebAuthn/rootCertificates
...
# This variable controls the usage of IPv6 within maimail.
...
# Prevent netfilter from setting an iptables/nftables rule to isolate the maimail docker network - y/n
...
openssl req -x509 -newkey rsa:4096 -keyout data/assets/ssl-example/key.pem -out data/assets/ssl-example/cert.pem -days 365 -subj "/C=DE/ST=NRW/L=Willich/O=maimail/OU=maimail/CN=${MAILCOW_HOSTNAME}" -sha256 -nodes
...
if [[ $SKIP_BRANCH != "y" ]]; then
maimail_git_commit=$(git rev-parse origin/${git_branch})
maimail_git_commit_date=$(git log -1 --format=%ci @{upstream} )
else
maimail_git_commit=$(git rev-parse ${git_branch})
maimail_git_commit_date=$(git log -1 --format=%ci @{upstream} )
